...
class MarginfiAccount implements MarginfiAccountType {
	...
	static async fetch(
		...
		const data: MarginfiAccountRaw =
			await client.program.account.marginfiAccount.fetch(address);
	...
	getBalance(bankPk: PublicKey): Balance {
		return (
			this.activeBalances.find((b) => b.bankPk.equals(bankPk)) ??
			Balance.createEmpty(bankPk)
		);
	}
	...
	computeMaxBorrowForBank(
		...
		debug(
			"attemptingToBorrowIsolatedAssetWithActiveDebt: %s",
		...
		debug(
			"attemptingToBorrowNewAssetWithExistingIsolatedDebt: %s",
		...
		if (
			...
			return this.computeMaxWithdrawForBank(
				banks,
				oraclePrices,
				bankAddress,
				opts,
	...
	computeMaxWithdrawForBank(
		...
		const initAssetWeight = bank.getAssetWeight(
			MarginRequirementType.Initial,
			priceInfo,
		);
		const maintAssetWeight = bank.getAssetWeight(
		...
		if (initAssetWeight.isZero() && !maintAssetWeight.isZero()) {
			if (liabilitiesInit.eq(0)) {
				...
				const { liabilities: maintLiabilities, assets: maintAssets } =
					this.computeHealthComponents(MarginRequirementType.Maintenance);
	...
	public computeLiquidationPriceForBank(
		...
		if (isLending) {
			...
			const liabWeight = bank.getLiabilityWeight(
				MarginRequirementType.Maintenance,
			);
			const priceConfidence = getPrice(
				priceInfo,
				PriceBias.Highest,
				false,
			).minus(getPrice(priceInfo, PriceBias.None, false));
	...
	computeMaxLiquidatableAssetAmount(
		...
		const liabilityPriceInfo = oraclePrices.get(
			liabilityBankAddress.toBase58(),
		);
		if (!liabilityPriceInfo)
			throw Error(
				`Price info for ${liabilityBankAddress.toBase58()} not found`,
			);
		...
		debug(
			"Underwater maint usd to adjust: $%d",
			underwaterMaintUsdValue.toFixed(6),
		);
		...
		const liabilitiesAmountUi =
			liabilityBalance.computeQuantityUi(liabilityBank).liabilities;
	...
	async makeDepositIx(
		...
		if (bank.mint.equals(NATIVE_MINT) && wrapAndUnwrapSol) {
			depositIxs.push(
				...makeWrapSolIxs(
					this.authority,
					new BigNumber(amount).minus(wSolBalanceUi),
				),
			);
		}
	...
	async makeRepayIx(
		...
		if (repayAll && !bank.emissionsMint.equals(PublicKey.default)) {
			repayIxs.push(
				...(
					await this.makeWithdrawEmissionsIx(
						program,
						banks,
						mintDatas,
						bankAddress,
	...
	async makeWithdrawIx(
		...
		if (
			withdrawAll &&
			!bank.emissionsMint.equals(PublicKey.default) &&
	...
	async makeBorrowIx(
		...
		const healthAccounts = this.getHealthCheckAccounts(
			bankMap,
			[bankAddress],
			[],
		);
	...
	async makeLendingAccountLiquidateIx(
		...
		if (!liabilityBank)
			throw Error(
				`Liability bank ${liabilityBankAddress.toBase58()} not found`,
			);
		const liabilityMintData = mintDatas.get(liabilityBankAddress.toBase58());
		if (!liabilityMintData)
			throw Error(
				`Mint data for bank ${liabilityBankAddress.toBase58()} not found`,
			);

		let ixs = [];

		const healthAccounts = [
			...this.getHealthCheckAccounts(
				bankMap,
				[liabilityBankAddress, assetBankAddress],
				[],
			),
	...
	projectActiveBalancesNoCpi(
		program: MarginfiProgram,
		instructions: TransactionInstruction[],
	): PublicKey[] {
		let projectedBalances = [
			...this.balances.map((b) => ({ active: b.active, bankPk: b.bankPk })),
		];
	...
	public describe(banks: BankMap, oraclePrices: OraclePriceMap): string {
		...
		const { assets: assetsMaint, liabilities: liabilitiesMaint } =
			this.computeHealthComponents(MarginRequirementType.Maintenance);
		const { assets: assetsInit, liabilities: liabilitiesInit } =
			this.computeHealthComponents(MarginRequirementType.Initial);
		let description = `
