...
class MarginfiAccountWrapper {
	...
	static async fetch(
		...
		const accountData = await MarginfiAccountWrapper._fetchAccountData(
			_marginfiAccountPk,
			config,
			program,
			commitment,
		);
		const marginfiAccount = MarginfiAccount.fromAccountParsed(
			_marginfiAccountPk,
			accountData,
		);

		const marginfiAccountProxy = new MarginfiAccountWrapper(
			_marginfiAccountPk,
			client,
			marginfiAccount,
		);
	...
	static fromAccountParsed(
		...
		return new MarginfiAccountWrapper(
			_marginfiAccountPk,
			client,
			marginfiAccount,
		);
	...
	static fromAccountDataRaw(
		...
		const marginfiAccountData = MarginfiAccount.decode(
			marginfiAccountRawData,
			idl,
		);
		return MarginfiAccountWrapper.fromAccountParsed(
			marginfiAccountPk,
			client,
			marginfiAccountData,
		);
	...
	public async simulateHealthCache(): Promise<MarginfiAccountWrapper> {
		const { marginfiAccount: account } =
			await this._marginfiAccount.simulateHealthCache(
				this._program,
				this.client.banks,
	...
	public canBeLiquidated(): boolean {
		const debugLogger = require("debug")(
			`mfi:margin-account:${this.address.toString()}:canBeLiquidated`,
		);
		const { assets, liabilities } =
			this._marginfiAccount.computeHealthComponents(
	...
	public computeHealthComponentsWithoutBiasLegacy(
		marginRequirement: MarginRequirementType,
	): {
		assets: BigNumber;
		liabilities: BigNumber;
	...
	public computeMaxWithdrawForBank(
		bankAddress: PublicKey,
	...
	public computeMaxLiquidatableAssetAmount(
	...
	public computeNetApy(): number {
		return this._marginfiAccount.computeNetApy(
			this.client.banks,
			this.client.oraclePrices,
		);
	}
	...
	public computeLoopingParams(
		...
		const depositPriceInfo = this.client.oraclePrices.get(
		...
		const borrowPriceInfo = this.client.oraclePrices.get(
	...
	async movePosition(
		...
		const debug = require("debug")(
			`mfi:margin-account:${this.address.toString()}:move-position`,
		);
		debug("Moving position from %s marginfi account", this.address.toBase58());

		const { transactions, actionTxIndex } = await this.makeMovePositionTx(
			amount,
			bankAddress,
			destinationAccountPk,
		);
	...
	async makeMovePositionTx(
		...
		const destinationAccount = await MarginfiAccountWrapper.fetch(
			destinationAccountPk,
			this.client,
		);

		const depositIx = await destinationAccount.makeDepositIx(
			amount,
			bankAddress,
		);
	 ...
	 */
	async repayWithCollatV2(
		props: RepayWithCollateralProps,
	): Promise<TransactionSignature[]> {
		const debug = require("debug")(
			`mfi:margin-account:${this.address.toString()}:repay`,
		);
		debug(
	...
	async makeRepayWithCollatTxV2({
		...
		const setupIxs = await this.makeSetupIx([
			borrowBankAddress,
			depositBankAddress,
		]);
		const cuRequestIxs =
		...
		const { instructions: updateFeedIxs, luts: feedLuts } =
			await this.makeUpdateFeedIx([depositBankAddress, borrowBankAddress]);
		...
		const clientLookupTables = await getClientAddressLookupTableAccounts(
			this.client,
		);
		...
		const canBeDownsized =
			txToManyKeys && txToBig && txSize - PRIORITY_TX_SIZE <= MAX_TX_SIZE;
	...
	async loopV2(props: LoopProps): Promise<TransactionSignature[]> {
		...
		const depositBank = this.client.banks.get(
			props.depositBankAddress.toBase58(),
		);
		if (!depositBank) throw Error("Deposit bank not found");
		const borrowBank = this.client.banks.get(
			props.borrowBankAddress.toBase58(),
		);
	...
	async makeLoopTxV2({
		...
		// creates atas if needed
		const setupIxs = await this.makeSetupIx(
			setupBankAddresses ?? [borrowBankAddress, depositBankAddress],
		);
		const cuRequestIxs =
			this.makeComputeBudgetIx().length > 0
				? this.makeComputeBudgetIx()
		...
		if (depositBank.mint.equals(NATIVE_MINT) && inputDepositAmount) {
			setupIxs.push(
				...makeWrapSolIxs(this.authority, new BigNumber(inputDepositAmount)),
			);
		}
	...
	async makeDepositStakedTx(
		...
		// fetch account info
		const [lstAccInfo, stakeAccountInfo, stakeAccInfoParsed] =
			await Promise.all([
		...
		const [rentExemptReserve, minimumDelegation] = await Promise.all([
			this._program.provider.connection.getMinimumBalanceForRentExemption(
				StakeProgram.space,
			),
		...
		const stakeAccLamports = Number(
			stakeAccParsed.parsed.info.stake?.delegation?.stake ?? 0,
		);
		const isFullStake = amountLamports >= stakeAccLamports;

		// calculate pool tokens
		const poolStakeAccLamports =
			(await this._program.provider.connection.getAccountInfo(poolStakeAddress))
				?.lamports ?? 0;
		const prePoolStake = Math.max(
			poolStakeAccLamports - minimumDelegation - rentExemptReserve,
			0,
		);

		const tokenSupply = parseInt(
			(await this._program.provider.connection.getTokenSupply(lstMint)).value
				.amount,
			10,
		);
		...
		const marginfiDepositIxs = await this.makeDepositIx(
			newPoolTokens / 1e9,
			bankAddress,
			depositOpts,
		);
	...
	async deposit(
		...
		const debug = require("debug")(
			`mfi:margin-account:${this.address.toString()}:deposit`,
		);
		debug(
			"Depositing %s into marginfi account (bank: %s)",
			amount,
			shortenAddress(bankAddress),
		);
	...
	async simulateBorrowLendTransaction(
		...
		if (healthSimOptions?.enabled) {
			...
			const updateFeedIx = await this.makeUpdateFeedIx(
			...
			const blockhash = (
				await this.client.provider.connection.getLatestBlockhash("confirmed")
			).blockhash;
		...
		const [mfiAccountData, ...bankData] =
			await this.client.simulateTransactions(
				[...txs, ...additionalTxs],
				[this.address, ...banksToInspect],
			);

		if (!mfiAccountData) throw new Error("Failed to simulate");
		const mfiAccount = MarginfiAccount.decode(
			mfiAccountData,
			this._program.idl,
		);
	...
	async makeRepayIx(
		...
		const tokenProgramAddress = this.client.mintDatas.get(
			bankAddress.toBase58(),
		)?.tokenProgram;
	...
	async makeRepayTx(
		...
		const ixs = await this.makeRepayIx(
			amount,
			bankAddress,
			repayAll,
			repayOpts,
	...
	async makeWithdrawStakedTx(
		...
		const { instructions: updateFeedIxs, luts: feedLuts } =
			await this.makeUpdateFeedIx(solBank?.address ? [solBank.address] : []);
		...
		// derive addresses
		const pool = findPoolAddress(
			new PublicKey(bankMetadata.validatorVoteAccount),
		);
		const lstMint = findPoolMintAddress(pool);
		...
		// delegate stake account
		const withdrawStakeIx: TransactionInstruction =
		...
		const crankTxn = addTransactionMetadata(
			new VersionedTransaction(crankMessage),
			{
				addressLookupTables: feedLuts,
				type: TransactionType.CRANK,
			},
		...
		const withdrawTxn = addTransactionMetadata(
			new VersionedTransaction(withdrawMessage),
			{
				signers: withdrawIxs.keys,
				addressLookupTables: this.client.addressLookupTables,
		...
		const stakeTxn = addTransactionMetadata(
			new VersionedTransaction(stakeMessage),
			{
		...
		return {
			transactions: [crankTxn, withdrawTxn, stakeTxn],
			actionTxIndex: 2,
		};
	}
	...
	async makeWithdrawAllTx(
		...
		const debug = require("debug")(
			`mfi:margin-account:${this.address.toString()}:withdraw`,
		);
		...
		const filteredSigners = withdrawIxsWrapped
			...
			.filter(
				(key, index, self) =>
					index === self.findIndex((k) => k.publicKey.equals(key.publicKey)),
			);
	...
	async withdraw(
		...
		const { transactions } = await this.makeWithdrawTx(
			amount,
			bankAddress,
			withdrawAll,
			withdrawOpts,
	...
	async borrow(
		...
		const debug = require("debug")(
			`mfi:margin-account:${this.address.toString()}:borrow`,
		);
		debug("Borrowing %s from marginfi account", amount);

		const { transactions } = await this.makeBorrowTx(
			amount,
			bankAddress,
			borrowOpts,
		);
	...
	async makeBorrowTx(
		...
		const { instructions: updateFeedIxs, luts: feedLuts } =
			await this.makeUpdateFeedIx([bankAddress]);
	...
	async makeWithdrawEmissionsTx(
		...
		const emissionsTx = addTransactionMetadata(
			new VersionedTransaction(
				new TransactionMessage({
					instructions: withdrawEmissionsIxs
	...
	async withdrawEmissions(
		...
		const debug = require("debug")(
			`mfi:margin-account:${this.address.toString()}:withdraw-emissions`,
		);
		debug(
			"Withdrawing emission from marginfi account (bank: %s)",
			bankAddresses.map((b) => b.toBase58()).join(", "),
		);

		const withdrawEmissionsTx =
			await this.makeWithdrawEmissionsTx(bankAddresses);
	...
	public async makeLendingAccountLiquidateIx(
		...
		const liabTokenProgramAddress = this.client.mintDatas.get(
			liabBankAddress.toBase58(),
		)?.tokenProgram;
	...
	public async lendingAccountLiquidate(
		...
		const debug = require("debug")(
			`mfi:margin-account:${this.address.toString()}:liquidation`,
		);
		debug(
			"Liquidating marginfi account %s",
			liquidateeMarginfiAccount.address.toBase58(),
		);
	 ...
	 */
	public async makeBeginFlashLoanIx(
		endIndex: number,
		authority?: PublicKey,
	): Promise<InstructionsWrapper> {
	...
	public async flashLoan(
		...
		const debug = require("debug")(
			`mfi:margin-account:${this.address.toString()}:flashLoan`,
		);
	...
	public async buildFlashLoanTx(
		...
		const projectedActiveBalances: PublicKey[] =
			this._marginfiAccount.projectActiveBalancesNoCpi(this._program, args.ixs);
	...
	async makeUpdateFeedIx(
		...
		const activeBanks = activeBanksPk.map(
			(pk) => this.client.banks.get(pk.toBase58())!,
		);
		const newBanks = newBanksPk.map(
			(pk) => this.client.banks.get(pk.toBase58())!,
		);

		const swbPullBanks = [
			...new Set([...activeBanks, ...newBanks]).values(),
		].filter((bank) => bank.config.oracleSetup === OracleSetup.SwitchboardPull);

		if (swbPullBanks.length > 0) {
			const filteredSwbPullBanks = swbPullBanks
				.filter((bank) => {
					// filter if isolated and collateral
					const activeBalance = this._marginfiAccount.balances.find((balance) =>
						balance.bankPk.equals(bank.address),
					);

					if (
						activeBalance?.assetShares.gt(new BigNumber(0)) &&
				...
				.filter((bank) => {
					// filter 0 feeds
					return !bank.oracleKey.equals(
						new PublicKey("DMhGWtLAKE5d56WdyHQxqeFncwUeqMEnuC2RvvZfbuur"),
					);
				})
			...
			if (filteredSwbPullBanks.length > 0) {
				...
				const pullFeedInstances: PullFeed[] = filteredSwbPullBanks.map(
					(pubkey) => new PullFeed(swbProgram, pubkey),
				);
				const crossbarClient = new CrossbarClient(
					process.env.NEXT_PUBLIC_SWITCHBOARD_CROSSSBAR_API ||
						"https://34.97.218.183.sslip.io",
				);
				const gateway =
					await pullFeedInstances[0].fetchGatewayUrl(crossbarClient);
	...
	async reload() {
		require("debug")(
			`mfi:margin-account:${this.address.toBase58().toString()}:loader`,
		)("Reloading account data");
		const marginfiAccountAi =
			await this._program.account.marginfiAccount.getAccountInfo(this.address);
		if (!marginfiAccountAi)
			throw new Error(
				`Failed to fetch data for marginfi account ${this.address.toBase58()}`,
			);
		const marginfiAccountParsed = MarginfiAccount.decode(
	...
	private _updateFromAccountParsed(data: MarginfiAccountRaw) {
		this._marginfiAccount = MarginfiAccount.fromAccountParsed(
			this.address,
			data,
		);
	}
	...
	public describe(): string {
		return this._marginfiAccount.describe(
			this.client.banks,
			this.client.oraclePrices,
		);
	}
